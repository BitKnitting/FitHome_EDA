# ###################################################
# bokeh_make_interactive_plot assumes an HTTP client 
# that is running on the machine running the bokeh server
# this is not our set up on the Rasp Pi (we run headless).
# so it makes it harder to debug.  We debugged this on our
# mac.
# ####################################################
...
import pandas as pd
from readings import PowerReadings
import time
import errors
...
class PowerPlot:
    ...
    def _draw_plot_with_date_slider(self, df):
        ...
        curdoc().add_root(self.date_slider)
        # Stylize the plot.
        self.f = self._stylize_plot(self.isodays[0])
        self.f.line(x='x', y='y', source=self.source)
        curdoc().add_root(self.f)
